...
def get_email_provider() -> str:
    ...
    return (_storage.get_secret("EMAIL_PROVIDER") or "gmail").lower()
...
def create_email_service() -> GoogleGmailService | ImapEmailService:
    ...
    if provider == "imap":
        ...
        return ImapEmailService(
            imap_host=_storage.get_secret("IMAP_HOST") or "localhost",
            imap_port=int(_storage.get_secret("IMAP_PORT") or "1143"),
            smtp_host=_storage.get_secret("SMTP_HOST") or "localhost",
            smtp_port=int(_storage.get_secret("SMTP_PORT") or "1025"),
            username=username,
            password=password,
            use_ssl=(_storage.get_secret("IMAP_USE_SSL") or "false").lower() == "true",
            archive_folder=_storage.get_secret("IMAP_ARCHIVE_FOLDER") or "Archive",
            trash_folder=_storage.get_secret("IMAP_TRASH_FOLDER") or "Trash",
        )

    # Default: Gmail
    access_token = _storage.get_secret("GMAIL_ACCESS_TOKEN")
    refresh_token = _storage.get_secret("GMAIL_REFRESH_TOKEN")
